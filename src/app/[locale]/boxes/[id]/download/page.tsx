...
export async function generateMetadata({ params }: DownloadPageProps): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: locale === 'zh-TW' ? `下載 ${box.name}` : locale === 'en-US' ? `Download ${box.name}` : `下載 ${box.name}`,
      description: locale === 'zh-TW' ? `下載${box.name}遊戲盒子` : locale === 'en-US' ? `Download the ${box.name} game box` : `下載${box.name}游戏盒子`,
    }
  } catch {
    return {
      title: locale === 'zh-TW' ? '下載遊戲盒子' : locale === 'en-US' ? 'Download Game Box' : '下載游戏盒子',
      description: locale === 'zh-TW' ? '下載遊戲盒子' : locale === 'en-US' ? 'Download a game box' : '下載游戏盒子',
    }
  }
}
