...
│   │   ├── layout.tsx              # 根布局
│   │   ├── page.tsx                # 首页（zh-CN，无前缀）
│   │   ├── globals.css             # 全局样式
│   │   ├── robots.ts               # robots.txt 生成器
│   │   ├── sitemap.xml/            # Sitemap 索引路由
│   │   ├── api/                    # API 路由
│   │   │   ├── sitemap/            # Sitemap 数据接口
│   │   │   └── revalidate/         # ISR 重新验证接口
│   │   ├── boxes/                  # 盒子大全（zh-CN）
│   │   │   ├── page.tsx
...
│   │   │   └── search.ts
│   │   └── sitemap/
│   │       └── config.ts           # Sitemap 配置（允许域名、默认域名等）
│   ├── lib/                        # 工具库
│   │   ├── api.ts                  # API 客户端（统一请求封装）
│   │   ├── api-types.ts            # API 请求/响应类型
│   │   ├── sitemap/                # Sitemap 生成逻辑
│   │   ├── format.ts               # 日期/数字格式化
│   │   ├── metadata.ts             # 动态 SEO meta 生成
│   │   └── utils.ts                # 通用工具函数
