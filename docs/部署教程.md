...
> **注意：** `NEXT_PUBLIC_API_KEY` 等敏感值请通过 Cloudflare Dashboard 的 **Workers → Settings → Variables** 设置，不要写入 `wrangler.toml`。
...
### 5.2 Cloudflare Workers 生产环境

在 Cloudflare Dashboard → Workers → Settings → Variables 中配置（或写入 `wrangler.toml` 的 `[vars]` 节，敏感值推荐用 Dashboard 配置）：

```bash
NEXT_PUBLIC_API_URL=https://api.yourdomain.com
NEXT_PUBLIC_SITE_ID=1
NEXT_PUBLIC_API_KEY=your-api-key        # 建议在Dashboard加密设置
REVALIDATE_SECRET=your-revalidate-secret # 建议在Dashboard加密设置
```

### 5.3 VPS + nginx 反向代理方案

如使用 VPS 服务器配合 nginx 做反向代理（如 1Panel），需确保 nginx 传递真实域名：

```nginx
# nginx 配置示例
proxy_set_header Host $ORIGIN_WEB;
# 将用户真实访问域名通过 X-Forwarded-Host 传递给 Worker
proxy_set_header X-Forwarded-Host $host;
```

前端已内置 `X-Forwarded-Host` 感知逻辑，Sitemap 会自动使用真实域名（而非内部源站域名）。详见 `src/lib/sitemap/security.ts`。
